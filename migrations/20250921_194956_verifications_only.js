// PocketBase migration: ensure/create "verifications" schema with verify_by
migrate(
  (db) => {
    ...
    try {
      employeesId = dao.findCollectionByNameOrId('employees').id
    } catch (e) {}

    let existing = null
    try {
      existing = dao.findCollectionByNameOrId('verifications')
    } catch (e) {}
    ...
    if (!existing) {
      ...
      const addFieldIfMissing = (field) => {
        if (!schema.getFieldByName(field.name)) {
  ...
  (db) => {
    const dao = new Dao(db)
    try {
      const existing = dao.findCollectionByNameOrId('verifications')
      const fld = existing.schema.getFieldByName('verify_by')
      if (fld) {
